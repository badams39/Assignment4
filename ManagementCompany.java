...
public class ManagementCompany {
     ...
     * @param taxID   tax Id
     * @param mgmFee  management Fee
     * @param x       x-coordinate for the plot
     * @param y       y-coordinate for the plot
    ...
    public int addProperty(Property property) {        
        ...
        for (int i = 0; i < properties.length; i++) {
            if (properties[i] == null) {
                properties[i] = property;  // Insert property into array
                numberOfProperties++;      // Increment the property count
                return i;  // Return the index where the property was added
            } else if (properties[i].getPlot().overlaps(property.getPlot())) {
                return -4; // Property's plot overlaps another property
            }
        }

        return -1;  // Fallback if for some reason no properties were added
    }
    ...
    public void removeLastProperty() {
        for (int i = properties.length - 1; i >= 0; i--) {
            if (properties[i] != null) {
                properties[i] = null;  // Remove the property
                numberOfProperties--;  // Decrement the property count
                break;
            }
        }
    }
    ...
    public String toString() {
        ...
        StringBuilder pStr = new StringBuilder("List of the properties for " + name + ", " + "taxID: " + taxID + "\n");
        pStr.append("______________________________________________________\n");

        for (int i = 0; i < properties.length && properties[i] != null; i++) {
            pStr.append(properties[i]).append("\n");
        }
